...
function Pago(props) {
  ...
  const handleSubmit = async (e)=>{
    ...
    if(dataLista){
       ...
       try {
        await fetch('http://18.228.212.190:5000/actualizar/respuestas', {
            method: "POST", 
             headers: {
  ...
  return (
    ...
    <span className='text-lg md:text-xl lg:text-2xl text-black'>Pago</span>
    <form className='grid grid-cols-1 lg:grid-cols-2 items-center justify-center h-full w-full gap-2  p-4'>
     ...
     <div className='flex flex-col lg:col-span-2 col-span-1'>
      ...
      <textarea 
      onChange={handle}
      className='h-[30vh] text-black border-2 border-gray-200 rounded-lg text-sm md:text-md lg:text-lg' defaultValue={props.data}></textarea>
     </div>
